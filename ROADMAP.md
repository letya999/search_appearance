...
- [x] Компонент `SearchProgress.tsx` (интегрирован в App.tsx)
  - [x] Прогресс-бар по этапам
  - [x] Таймер прошедшего времени (частично, визуализировано в статусе)
  - [ ] ETA до завершения
  - [x] Анимация текущего этапа
...
### 8.2 Дедупликация
- [x] Проверка дубликатов при загрузке (уже есть!)
- [x] Перцептивный хэш (pHash) для быстрой проверки

### 8.3 Async оптимизации
- [ ] Параллельный анализ нескольких фото
- [x] Streaming ответов от VLM (via WebSocket)
